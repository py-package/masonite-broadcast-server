...
export class BroadcastServer {
    ...
    public defaultOptions: any = {
        ...
        databaseConfig: {
            redis: {},
            sqlite: {
                databasePath: '/database/masonite-broadcast-server.sqlite'
            }
        },
        devMode: false,
        host: null,
     ...
     */
    startup(): void {
        Log.title(`\nM A S O N I T E  B R O A D C A S T  S E R V E R\n`);
     ...
     */
    stop(): Promise<any> {
        console.log('Stopping the MASONITE BROADCAST SERVER')
        let promises = [];
        ...
        return Promise.all(promises).then(() => {
            this.subscribers = [];
            console.log('The MASONITE BROADCAST SERVER server has been stopped.');
        });
    }
